...
const handleFavorite =(id)=>{
  toast.success(`${title} is added as favorite recipe! `);
  const storedIds = JSON.parse(localStorage.getItem('selectedElementIds')) || [];
...
const removeFavorite=(id)=>{
  ...
  toast.success(`${title} is removed from favorites successfully!!!!`)
        ...
        <p className="card-text">Ratings: {rating}/5</p>
        {
         ...
         <div className="">
          <span className='me-3'>Remove favorite</span>
              <button onClick={()=>removeFavorite(recipe_id) } className='btn p-0 text-danger '> <FontAwesomeIcon icon={faHeart}/></button><ToastContainer/>
         </div> )
        }
      </div>
    </div>
